...
border bb(t=0,0.5){x=0.5; y=t; label=2;}; 
border bc(t=0,1.0){x=0.5-t; y=0.5; label=1;}; 
border bd(t=0,1.0){x=-0.5; y=0.5-t; label=2;}; 
border be(t=0,0.5){x=t-0.5; y=-0.5; label=1;}; 
border bf(t=0,0.5){x=0.0; y=t-0.5; label=2;}; 
...
fespace Sh(Th,P2); 
fespace Vh(Th,P1);  
Sh u1,u2,H1,H2,v1,v2,w1,w2,u10,u20,H10,H20,varphi,unused,phi,F; 
Vh p,q; 
...
+q*(dx(u1)+dy(u2))  
+1.0e-8*p*q)       
-int2d(Th)((H10*w1+H20*w2)/dt+J*(dx(w2)-dy(w1))+f1*v1+f2*v2+(u10*v1+u20*v2)/dt+g*q)  
+on(1,u1=0.0,u2=0.0,H2=0.0)+on(2,u1=0.0,u2=0.0,H1=0.0);  
...
cout<<"L^2 error of H="<<sqrt(int2d(Th)(abs(H1e-H1)^2+abs(H2e-H2)^2))<<endl;
