...
fespace Sh(Th,P2); 
fespace Vh(Th,P1);  
Sh u1,u2,A,B,v1,v2,a,wh,u10,u20,A0,varphi,phi;  
Vh p,q; 
Sh varphiR;
...
//Compute the harmonic function varphi
problem VPHI(varphi,phi,solver=sparsesolver) //solver=GMRES
=int2d(Th)(dx(varphi)*dx(phi)+dy(varphi)*dy(phi))      
+on(1,2,varphi=1.0)+on(3,4,varphi=0.0);  
VPHI;

//Compute a surrogate for the exact harmonic function varphi by using a much smaller mesh 
fespace Sh2(Th2,P2);
Sh2 varphi2,phi2;  
problem VPHI2(varphi2,phi2,solver=sparsesolver) 
...
problem BackEuler([A,u1,u2,B,p],[a,v1,v2,wh,q],solver=sparsesolver) 
=int2d(Th)(A*a/dt+(u1*v1+u2*v2)/dt
+(dx(A)*dx(a)+dy(A)*dy(a))
+(u1*(dx(A0)+dx(varphi))+u2*(dy(A0)+dy(varphi)))*a 
...
-p*dx(v1)-p*dy(v2) 
+(dx(A0)+dx(varphi))*B*v1+(dy(A0)+dy(varphi))*B*v2
+B*wh+dx(A)*dx(wh)+dy(A)*dy(wh) 
+q*(dx(u1)+dy(u2))  
+1.0e-8*p*q)       
-int2d(Th)((A0/dt+J+u1e*(dx(varphiR)+dy(varphiR)))*a
+(f1-dx(varphiR)*curlH)*v1+(f2-dy(varphiR)*curlH)*v2+(u10*v1+u20*v2)/dt+g*q)  
+on(1,2,3,4,A=0.0,u1=0.0,u2=0.0,B=0.0);  
...
cout<<"L^2 error of H="<<sqrt(int2d(Th)(abs(H1e+dy(varphiR)-dy(A)-dy(varphi))^2
+abs(H2e-dx(varphiR)+dx(varphi)+dx(A))^2))<<endl;
